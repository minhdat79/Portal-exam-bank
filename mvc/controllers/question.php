...
class Question extends Controller
{
        ...
        if (AuthCore::checkPermission("cauhoi", "create")) {
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                ...
                foreach ($questions as $question) {
                    ...
                    $result = $this->cauHoiModel->create($noidung, $level, $chude, $chuong, $nguoitao);
                    $macauhoi = mysqli_insert_id($result);
                    $index = 1;
    ...
    public function getQuestionBySubject()
    {
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $content = $_POST['content'];
            $page = $_POST['page'];
            $result = $this->cauHoiModel->getQuestionBySubject($machude,$machuong,$dokho,$content,$page);
            echo json_encode($result);
        }
    }
    ...
    public function getTotalPageQuestionBySubject()
    {
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $dokho = $_POST['dokho'];
            $content = $_POST['content'];
            $result = $this->cauHoiModel->getTotalPageQuestionBySubject($machude,$machuong,$dokho,$content);
            echo $result;
        }
    }
    ...
    public function getsoluongcauhoi()
    {
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $chude = $_POST['chude'];
            $dokho = $_POST['dokho'];
            $result = $this->cauHoiModel->getsoluongcauhoi($chuong,$chude,$dokho);
            echo $result;
        }
    }
}
