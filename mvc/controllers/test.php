...
class Test extends Controller
{
 ...
 // Thay thế hàm taketest() trong test.php bằng đoạn này:
    ...
    public function taketest($made)
    {
        if(filter_var($made, FILTER_VALIDATE_INT) !== false) {
            if (AuthCore::checkPermission("tgthi", "join")) {
                ...
                // Chỉ kiểm tra trong khoảng thời gian thi, KHÔNG giới hạn thời gian cá nhân
    ...
    public function addTest()
    {
        if ($_SERVER["REQUEST_METHOD"] == "POST" && AuthCore::checkPermission("dethi", "create")) {
            ...
            $result = $this->dethimodel->create($machude, $nguoitao, $tende, $thoigianthi, $thoigianbatdau, $thoigianketthuc, $xembailam, $xemdiem, $xemdapan, $daocauhoi, $daodapan, $tudongnop, $loaide, $socaude, $socautb, $socaukho, $chuong, $maphong);
            echo $result;
        }
    }
    ...
    public function updateTest()
    {
        if ($_SERVER["REQUEST_METHOD"] == "POST" && AuthCore::checkPermission("dethi", "update")) {
            ...
            $tudongnop = $_POST['tudongnop'];
            $maphong = $_POST['maphong'];
            $result = $this->dethimodel->update($made, $machude, $tende, $thoigianthi, $thoigianbatdau, $thoigianketthuc, $xembailam, $xemdiem, $xemdapan, $daocauhoi, $daodapan, $tudongnop, $loaide, $socaude, $socautb, $socaukho, $chuong, $maphong);
            echo $result;
        }
    }
    ...
    public function getTestGroup()
    {
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            $maphong = $_POST['maphong'];
            $result = $this->dethimodel->getListTestGroup($maphong);
            echo json_encode($result);
        }
    }
    ...
    public function getExamineeByGroup()
    {
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            $made = $_POST['made'];
            $maphong = $_POST['maphong'];
            $result = $this->ketquamodel->getExamineeByGroup($made, $maphong);
            echo json_encode($result);
        }
    }
    ...
    public function getStatictical()
    {
        $made = $_POST['made'];
        $maphong = $_POST['maphong'];
        $result = $this->ketquamodel->getStatictical($made, $maphong);
        echo json_encode($result);
    }
            ...
            <table style="width:100%">
                <tr>
                    ...
                    <td style="text-align: center;">
                        ...
                        <p style="font-weight:bold">Học phần: ' . $info['tenchude'] . '</p>
                        <p style="font-weight:bold">Mã học phần: ' . $info['machude'] . '</p>
    ...
    public function exportExcel()
    {
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $maphong = $_POST['maphong'];
            $ds = $_POST['ds'];
            $result = $this->ketquamodel->getTestScoreGroup($made,$maphong);
            if($maphong == 0){
    ...
    public function getMarkOfAllTest()
    {
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
        $maphong = $_POST['maphong'];
        $result = $this->ketquamodel->getMarkOfAllTest($maphong);
        $excel = new PHPExcel();
        //Chọn trang cần ghi (là số từ 0->n)
        $excel->setActiveSheetIndex(0);
        //Tạo tiêu đề cho trang. (có thể không cần)
    ...
    public function getTestsGroupWithUserResult()
    {
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            $maphong = $_POST['maphong'];
            $result = $this->dethimodel->getTestsGroupWithUserResult($maphong, $_SESSION['user_id']);
            echo json_encode($result);
        }
    }
}