...
class KetQuaModel extends DB{
    ...
    {
        // Kiểm tra đã có bản ghi chưa, nếu rồi thì không insert thêm
        $sql_check = "SELECT * FROM `ketqua` WHERE `made`='$made' AND `manguoidung`='$manguoidung'";
        $result_check = mysqli_query($this->con, $sql_check);
        if (mysqli_fetch_assoc($result_check)) {
            return true; // Đã có rồi, không cần insert
        }
    ...
    public function getExamineeByGroup($made, $maphong) {
        $sql = "SELECT KQ.*, email, hoten, avatar FROM ketqua KQ, nguoidung ND, chitietphong CTN WHERE KQ.manguoidung = ND.id AND CTN.manguoidung = ND.id AND KQ.made = $made AND CTN.maphong = $maphong";
        $result = mysqli_query($this->con,$sql);
        $rows = array();
        while($row = mysqli_fetch_assoc($result)){
            $rows[] = $row;
    ...
    public function getMarkOfAllTest($maphong)
    {
        // Lấy danh sách đề thi
        $sql_giaodethi = "SELECT dethi.made,tende FROM giaodethi,dethi WHERE maphong = $maphong AND giaodethi.made = dethi.made";
        ...
        $sql_sinhvien = "SELECT id,hoten FROM nguoidung, chitietphong WHERE nguoidung.id = chitietphong.manguoidung AND chitietphong.maphong = $maphong";
        ...
        foreach($arr_dethi as $dethi) {
            $arr_ketqua[$dethi['made']] = $this->getMarkOfOneTest($maphong,$dethi['made']);
        }
    ...
    public function getMarkOfOneTest($maphong,$made)
    {
        $sql = "SELECT DISTINCT giaodethi.made,chitietphong.manguoidung,ketqua.diemthi
        FROM giaodethi, chitietphong LEFT JOIN ketqua ON chitietphong.manguoidung = ketqua.manguoidung AND ketqua.made = $made 
        WHERE giaodethi.maphong = chitietphong.maphong AND giaodethi.maphong = $maphong AND giaodethi.made = $made";
        $result = mysqli_query($this->con,$sql);
        $rows = array();
        while($row = mysqli_fetch_assoc($result)) {
            $rows[] = $row;
    ...
    {
        $sql = "SELECT DISTINCT ketqua.made, tende, tenchude, machude, thoigianthi, manguoidung, hoten, socaudung,(socaude + socautb + socaukho) AS tongsocauhoi , diemthi
        FROM chitietketqua, ketqua, dethi, chude, nguoidung
        WHERE chitietketqua.makq = '$makq' AND chitietketqua.makq = ketqua.makq AND ketqua.manguoidung = nguoidung.id AND ketqua.made = dethi.made AND dethi.machude = chude.machude";
        $result = mysqli_query($this->con, $sql);
        return mysqli_fetch_assoc($result);
    }

    // Lấy điểm để thống kê 
    public function getStatictical($made, $maphong)
    {
        $nhomm = $maphong != 0 ? "AND chitietphong.maphong = $maphong" : "";
        $sql = "SELECT chitietphong.manguoidung, ketqua.manguoidung AS mandkq, makq, ketqua.made, diemthi
        FROM chitietphong
        JOIN giaodethi ON chitietphong.maphong = giaodethi.maphong
        LEFT JOIN ketqua ON ketqua.manguoidung = chitietphong.manguoidung AND ketqua.made = '$made'
    ...
    public function getListAbsentFromTest($filter, $input, $args) {
        if (is_array($args['maphong'])) {
            $listGroup = implode(", ", $args['maphong']);
        } else {
            $listGroup = $args['maphong'];
        }
        $query = "SELECT KQ.makq, KQ.made, CTN.manguoidung, KQ.diemthi, KQ.thoigianvaothi, KQ.thoigianlambai, KQ.socaudung, KQ.solanchuyentab, email, hoten, avatar FROM chitietphong CTN JOIN nguoidung ND ON ND.id = CTN.manguoidung LEFT JOIN ketqua KQ ON CTN.manguoidung = KQ.manguoidung AND KQ.made = ".$args['made']." WHERE KQ.made IS NULL AND CTN.maphong IN ($listGroup)";
        return $query;
    }

    public function getQueryAll($filter, $input, $args) {
        $absent_query = $this->getListAbsentFromTest($filter, $input, $args);
        $query = "SELECT DISTINCT KQ.*, email, hoten, avatar FROM ketqua KQ, nguoidung ND, chitietphong CTN WHERE KQ.manguoidung = ND.id AND CTN.manguoidung = ND.id AND KQ.made = ".$args['made'];
        if (is_array($args['maphong'])) {
            $list = implode(", ", $args['maphong']);
            $query .= " AND CTN.maphong IN ($list)";
        } else {
            $query .= " AND CTN.maphong = ".$args['maphong'];
        }
        $present_query = $query;
    ...
    public function getTestScoreGroup($made,$maphong){
        $sql = "SELECT ds.manguoidung,ds.hoten,kqt.diemthi,kqt.thoigianvaothi,kqt.thoigianlambai,kqt.socaudung,kqt.solanchuyentab FROM (SELECT ctn.manguoidung,nd.hoten FROM chitietphong ctn JOIN nguoidung nd ON ctn.manguoidung=nd.id WHERE ctn.maphong=$maphong) ds LEFT JOIN 
        (SELECT kq.manguoidung,kq.diemthi,kq.thoigianvaothi,kq.thoigianlambai,kq.socaudung,kq.solanchuyentab FROM ketqua kq JOIN giaodethi gdt ON kq.made=gdt.made WHERE gdt.made=$made AND gdt.maphong=$maphong) kqt ON ds.manguoidung=kqt.manguoidung";
        $result = mysqli_query($this->con,$sql);
        $rows = array();
        while($row = mysqli_fetch_assoc($result)){
            $rows[] = $row;
    ...
    public function getTestAll($made,$ds){
        $list = implode(", ", $ds);
        $sql = "(SELECT KQ.makq, KQ.made, CTN.manguoidung, KQ.diemthi, KQ.thoigianvaothi, KQ.thoigianlambai, KQ.socaudung, KQ.solanchuyentab, email, hoten, avatar FROM chitietphong CTN JOIN nguoidung ND ON ND.id = CTN.manguoidung LEFT JOIN ketqua KQ ON CTN.manguoidung = KQ.manguoidung AND KQ.made = $made WHERE KQ.made IS NULL AND CTN.maphong IN ($list))
        UNION
        (SELECT DISTINCT KQ.*, email, hoten, avatar FROM ketqua KQ, nguoidung ND, chitietphong CTN WHERE KQ.manguoidung = ND.id AND CTN.manguoidung = ND.id AND KQ.made = $made  AND CTN.maphong IN ($list))
