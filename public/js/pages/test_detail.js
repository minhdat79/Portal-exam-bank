...
$(document).ready(function () {
  ...
  $(".filtered-by-group").click(function (e) {
    ...
    mainPagePagination.option.maphong = currentGroupID == 0 ? listGroupID.slice(1) : currentGroupID;
    resetFilterState();
  ...
  $("#export_excel").click(function () {
    let maphong = $(".filtered-by-group.active").data("value");
    let ds = mainPagePagination.option.maphong;
    console.log(maphong)
    console.log(ds)
    $.ajax({
      method: "post",
...
function getStatictical() {
  $.ajax({
    ...
    data: {
      made: made,
      maphong: $(".filtered-by-static.active").data("id"),
    },
    dataType: "json",
    success: function (response) {
...
mainPagePagination.option.maphong = listGroupID.slice(1);
