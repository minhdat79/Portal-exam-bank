...
function getToTalQuestionOfChapter(chuong, chude, dokho) {
  var result = 0;
  $.ajax({
...
$(document).ready(function () {
  ...
  $("#phong-hp").on("change", function () {
    let index = $(this).val();
    let machude = groups[index].machude;
    showListGroup(index);
    showChapter(machude);
  });
  ...
  function showListGroup(index) {
    let html = ``;
    if (groups[index].phong.length > 0) {
      ...
      groups[index].phong.forEach((item) => {
        html += `<div class="col-4">
                    <div class="form-check">
                        <input class="form-check-input select-group-item" type="checkbox" value="${item.maphong}"
                            id="phong-${item.maphong}" name="phong-${item.maphong}">
                        <label class="form-check-label" for="phong-${item.maphong}">${item.tenphong}</label>
                    </div>
                </div>`;
      });
    } else {
    ...
    if ($(".form-taodethi").valid()) {
      if (getGroupSelected().length != 0) {
        $.ajax({
          ...
          url: "./test/addTest",
          data: {
            machude: groups[$("#phong-hp").val()].machude,
            tende: $("#name-exam").val(),
  ...
  function showInfo(dethi) {
    ...
    $.when(showGroup(), showChapter(dethi.machude)).done(function () {
      $("#phong-hp").val(findIndexGroup(dethi.phong[0])).trigger("change");
      setGroup(dethi.phong, dethi.thoigianbatdau);
      if (dethi.loaide == "1") {
  ...
  function findIndexGroup(maphong) {
    let i = 0;
    let index = -1;
    while (i <= groups.length && index == -1) {
      index = groups[i].phong.findIndex((item) => item.maphong == maphong);
      if (index == -1) i++;
    }
    return i;
  }
